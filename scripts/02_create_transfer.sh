...
createTransfer() {
  ...
  curl --fail -X POST \
    http://ml-api-adapter.local/transfers \
...
acceptTransfer() {
  ...
  curl --fail -X PUT \
    http://ml-api-adapter.local/transfers/${TRANSFER_ID} \
    -H 'Accept: application/vnd.interoperability.quotes+json;version=1' \
...
getPositions() {
  ...
  curl --fail -X GET \
    http://central-ledger.local/participants/payerfsp/positions
  ...
  curl --fail -X GET \
    http://central-ledger.local/participants/payeefsp/positions
